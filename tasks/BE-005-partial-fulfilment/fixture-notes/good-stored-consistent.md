...
on `Order` is updated at create, deliver **and** cancel, so the release works and every gate
passes. The other six files are byte-identical to `known-good`.
...
where the truth lives: here, three write sites in another package each keep a copy of a
value the shipments already determine; in `known-good`, one read. A rubric whose
