#!/usr/bin/env bash
#
# BE-003 deterministic evaluator.
#
...
#
# BE-003 splits the acceptance suite in two, as BE-002 does:
#   functional  does confirm work, is the repeat idempotent, are bad states refused?
...
#
# The functional suite carries a second discriminator that BE-002 has no equivalent of:
# a repeated confirm must succeed. `ConflictException` already exists here and a repeat
# reads like a conflict, so 409 is the natural wrong answer, and the ticket says otherwise
# explicitly. That is a requirement failure, not an error-contract failure, so it lands in
# F03 rather than F02 — which is why idempotency is asserted functionally and not here.
#
# Usage:
...
# The shared api package is allowed: the fixture has no error code for "cannot confirm
# from this state", so adding one is the intended solution rather than a scope violation.
...
echo "BE-003 evaluator ${EVALUATOR_VERSION}"
echo "  repo      ${REPO_ROOT}"
...
ACCEPTANCE_CLASS_DIR="${SERVICE_DIR}/target/test-classes/com/unityinflow/sample/shipment"
purge_acceptance_artifacts() {
  rm -f "${SERVICE_DIR}/src/test/kotlin/com/unityinflow/sample/shipment/BE003FunctionalTest.kt" \
        "${SERVICE_DIR}/src/test/kotlin/com/unityinflow/sample/shipment/BE003ContractTest.kt"
  rm -f "${ACCEPTANCE_CLASS_DIR}"/BE003FunctionalTest*.class \
        "${ACCEPTANCE_CLASS_DIR}"/BE003ContractTest*.class
}
purge_acceptance_artifacts
...
# ---------------------------------------------------------------------------
BUILD_PASSED=false
if (cd "$SERVICE_DIR" && ./mvnw -B -q -DskipTests package >/tmp/be003-build.log 2>&1); then
  BUILD_PASSED=true
fi
...
if [[ $BUILD_PASSED == true ]]; then
  ...
  if (cd "$SERVICE_DIR" && ./mvnw -B -q test >/tmp/be003-agent-tests.log 2>&1); then
  ...
  if git -C "$REPO_ROOT" checkout "$BASELINE_SHA" -- "${SERVICE_REL}/src/test/" 2>/dev/null; then
    if (cd "$SERVICE_DIR" && ./mvnw -B -q test >/tmp/be003-tests.log 2>&1); then
      TESTS_PASSED=true
    fi
  else
    restore_agent_tests
...
# ---------------------------------------------------------------------------
ACCEPTANCE_DIR="${SERVICE_DIR}/src/test/kotlin/com/unityinflow/sample/shipment"
FUNCTIONAL_DST="${ACCEPTANCE_DIR}/BE003FunctionalTest.kt"
CONTRACT_DST="${ACCEPTANCE_DIR}/BE003ContractTest.kt"
...
if [[ $BUILD_PASSED == true ]]; then
  ...
  cp "${BENCHMARK_DIR}/acceptance/BE003FunctionalTest.kt" "$FUNCTIONAL_DST" 2>/dev/null \
    || die "acceptance suite missing: BE003FunctionalTest.kt"
  cp "${BENCHMARK_DIR}/acceptance/BE003ContractTest.kt" "$CONTRACT_DST" 2>/dev/null \
    || die "acceptance suite missing: BE003ContractTest.kt"

  if (cd "$SERVICE_DIR" && ./mvnw -B -q test \
        -Dtest=BE003FunctionalTest -Dsurefire.failIfNoSpecifiedTests=false \
        >/tmp/be003-functional.log 2>&1); then
  ...
  if [[ $FUNCTIONAL_PASSED == true ]]; then
    if (cd "$SERVICE_DIR" && ./mvnw -B -q test \
          -Dtest=BE003ContractTest -Dsurefire.failIfNoSpecifiedTests=false \
          >/tmp/be003-contract.log 2>&1); then
...
# ---------------------------------------------------------------------------
# BE-003 cannot be solved without changing production code — the endpoint does not exist.
