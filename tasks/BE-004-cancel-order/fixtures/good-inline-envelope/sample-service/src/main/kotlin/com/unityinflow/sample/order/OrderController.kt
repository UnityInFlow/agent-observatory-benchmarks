...
class OrderController(
    ...
    fun cancel(@PathVariable orderId: String): ResponseEntity<Any> {
        ...
        return when (order.status) {
            ...
            OrderStatus.CANCELLED -> ResponseEntity.ok(order)
            ...
            OrderStatus.ACTIVE -> {
                    ...
                    return ResponseEntity.status(HttpStatus.CONFLICT).body(
                        ApiError(
                            ApiErrorBody(
                                ...
                                message = "Order '$orderId' has shipment '${accepted.shipmentId}' accepted by the carrier and cannot be cancelled",
                            ),
                        ),
                    )
                }
                ...
                ResponseEntity.ok(repository.save(order.copy(status = OrderStatus.CANCELLED)))
            }
        }
    }
}
